...
json::Value Proxy::public_request(std::string method, json::Value data) {
  ...
  curl_handle.setOpt(cURLpp::Optoins::WriteStream(&response));
  curl_handle.perform();
...
json::Value Proxy::private_request(Method method, std::string command, json::Value data) {
  ...
  if (method == GET) {
    ...
    if (method == POST) {
      if (debug) std::cerr << "SEND: POST" << curl << "( " request << " )" << std::endl;
      curl_handle.setOpt(new cURLpp::Options::Post(true));
    } else {
      if (debug) std::cerr << "SEND: PUT " << curl << "(" request << " )" << std::endl;
  ...
  if (debug) std::cerr << rsp << std::endl;
